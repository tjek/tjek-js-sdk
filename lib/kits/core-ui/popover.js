...
<div class="tjek-popover__background" data-close></div>
<div class="tjek-popover__menu">
    {{#header}}
        <div class="tjek-popover__header">{{header}}</div>
    {{/header}}
    <div class="tjek-popover__content">
        <ul>
            {{#singleChoiceItems}}
                <li data-index="{{index}}">
                    <p class="tjek-popover-item__title">{{item.title}}</p>
                    {{#item.subtitle}}
                        <p class="tjek-popover-item__subtitle">{{item.subtitle}}</p>
                    {{/item.subtitle}}
                </li>
            {{/singleChoiceItems}}
        </ul>
...
class Popover extends MicroEvent {
    ...
    render() {
        ...
        this.el.className = 'tjek-popover';
    ...
    position() {
        ...
        const menuEl = this.el.querySelector('.tjek-popover__menu');
    ...
    addEventListeners() {
        ...
        Gator(this.el).on('click', '.tjek-popover__menu', (e) => {
            e.stopPropagation();
        });
