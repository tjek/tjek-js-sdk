    ...
    <div class="sgn-shopping-popup">
        <div class="sgn-popup-header">
            <div class="sgn-popup-header-label">
                <span>{{locale.shoppingListLabel}}</span>
            </div>            
        ...
        <ol class="sgn-shopping-ol">
                ...
                <div class="sgn-shopping-li-flex-container">
                    <div class="sgn-shopping-li-div-text">
                        <div class="sgn-shopping-li-div-text-heading sgn-truncate-elipsis">
                            <span>{{locale.currencyPrefix}}{{price}} {{locale.currencySuffix}} - {{name}}</span><br/>
                        </div>
                    </div>
                </div>
            </li>
            ...
            <li class="sgn-shopping-li-crossed sgn-hide-print">
                <button class="sgn-shopping-clear-ticked-list-btn sgn-hide-print">{{locale.deleteCrossedOutItemsButton}}</button>
            </li>
            {{/hasTicked}}
        </ol>
        <div>
            <button class="sgn-shopping-clear-list-btn sgn-hide-print">{{locale.clearListButton}}</button>
            <button class="sgn-shopping-print-list-btn sgn-hide-print">{{locale.printButton}}</button>
        </div>
    </div>\
`;

const shoppingList = ({template, locale}) => {
    ...
    const formatListToShare = (data = []) => {
        ...
        notTicked.forEach((offer) => {
            notTickedStr += `[] ${_locale.currencyPrefix}${offer.price} ${_locale.currencySuffix} - ${offer.name}\n`;
        });

        ticked.forEach((offer) => {
            notTickedStr += `[x] ${_locale.currencyPrefix}${offer.price} ${_locale.currencySuffix} - ${offer.name}\n`;
        });
