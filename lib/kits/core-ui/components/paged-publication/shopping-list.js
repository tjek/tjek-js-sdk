...
const shoppingList = ({template}) => {
    ...
    const translations = {
        ...
        clearListButton: translate(
            'paged_publication_viewer_shopping_list_clear_button'
        ),
    ...
    const showShoppingList = () => {
        ...
        addEventListeners();
        addBlockerListenerTo(container);
    };
    ...
    const transformSavedOffers = (savedOffers = []) => {
        ...
        savedOffers.forEach((offer, index) => {
            const price = formatPrice(offer.price, localeCode, currency);
            newSavedOffers.push({
                index,
                ...offer,
                price
    ...
    const tickOffer = (e) => {
        ...
        storedPublicationOffers[index].is_ticked = storedPublicationOffers[
            index
        ].is_ticked
            ? false
            : true;
    ...
    const addShareListener = () => {
        ...
        const textData = formatListToShare(
    ...
    const formatListToShare = (data = []) => {
        let notTickedStr = '';
        const notTicked = data?.filter((offer) => !offer.is_ticked);
        const ticked = data?.filter((offer) => offer.is_ticked);

        notTicked.forEach((offer) => {
            notTickedStr += `[ ] ${offer.price} - ${offer.name}\n`;
        });

        ticked.forEach((offer) => {
            notTickedStr += `[x] ${offer.price} - ${offer.name}\n`;
        });
