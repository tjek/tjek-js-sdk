...
export default class MainContainer {
    ...
    setCustomStyles() {
        if (this.scriptEls.headerBackground) {
            ...
            const sgnPp = this.el.querySelector('.sgn__pp');
            sgnPp.style.backgroundColor = 'green !important';
        }

        if (this.scriptEls.hiddenComponents) {
            this.hideElements(this.scriptEls.hiddenComponents);
        }
    }

    hideElements(strElements = '') {
        const elementNames = strElements
            .replace(/ /g, '')
            .split(',')
            .filter((name) => name);
