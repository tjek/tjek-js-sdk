...
const MainContainer = ({
    ...
    const removeComponents = () => {
        const componentClassNames = scriptEls.removedComponents;
        ...
        if (scriptEls.offerClickBehavior === 'open-offer-link') {
            const shoppingBtn = el.querySelector('.sgn__offer-shopping');
            shoppingBtn?.parentNode?.removeChild(shoppingBtn);
        }
    };
    ...
    const addClosePubListener = () => {
        ...
        sgnPp.addEventListener(
            ...
            (e) => {
                if (e.keyCode === EscKey) {
                    const modalBlocker = document.querySelector('.sgn-blocker');
