...
import {translate, pushQueryParam} from './components/helpers/component';
...
const PagedPublication = (
    ...
    const start = async () => {
        ...
        displayQueryParamPageId();
    };

    const displayQueryParamPageId = () => {
        if (scriptEls.displayQueryParams) {
            const nextControl = options.el.querySelector(
                '.sgn-pp__control[data-direction=next]'
            );
            const prevControl = options.el.querySelector(
                ...
                pushQueryParam([
                    {
                        ...
                        val: sgnViewer._core.verso.position * 2
                    }
                ]);
            const prevAction = () =>
                pushQueryParam(
                    ...
                    sgnViewer._core.verso.position * 2 ? 'set' : 'delete'
                );
            ...
            options.el.addEventListener('keydown', (e) => {
                const {keyCode} = e;
