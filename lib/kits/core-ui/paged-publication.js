...
const PagedPublication = (
    ...
    const start = async () => {
        ...
        displayUrlParams();
        addHeaderControlsListener();
    };

    const addHeaderControlsListener = () => {
        if (scriptEls.showHeaderControls) {
            ...
            const controlDirectionObserver = new MutationObserver(
                (mutations) => {
                        ...
                        if (mutation.type === 'attributes') {
                            ...
                            if (element?.dataset?.direction === 'prev') {
                                element?.classList.contains('sgn-pp--hidden')
                                    ? firstControl.classList.add(
                                          'sgn-pp--hidden'
                                      )
                                    : firstControl.classList.remove(
                                          'sgn-pp--hidden'
                                      );
                            }
                            if (element?.dataset?.direction === 'next') {
                                element?.classList.contains('sgn-pp--hidden')
                                    ? lastControl.classList.add(
                                          'sgn-pp--hidden'
                                      )
                                    : lastControl.classList.remove(
                                          'sgn-pp--hidden'
                                      );
                            }
                        }
