...
const IncitoPublication = (
    ...
    const start = async () => {
        ...
        on(
            ...
            function (e) {
                ...
                const offerParams = {view_id: id, publication_id: options.id};

                dispatchOfferClickEvent(offerParams);
                clickOfferCell(offerParams);
            }
        );
    ...
    const dispatchOfferClickEvent = (detail) => {
        detail.fetchOffer = (callback) =>
            bootstrapper.fetchOffer(detail, callback);
    ...
    const clickOfferCell = async (offerParams) => {
        const shoppingBtn = options.el.querySelector('.sgn__offer-shopping');
        const {offer} = await bootstrapper.fetchOffer(offerParams);

        options?.eventTracker?.trackOfferOpened({
            'of.id': offer.id,
            vt: options.eventTracker.createViewToken(offer.id)
        });
