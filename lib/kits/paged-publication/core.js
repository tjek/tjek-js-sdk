...
class PagedPublicationCore extends MicroEvent {
    ...
    destroy() {
        ...
        delete this.els.root.dataset.started;
        delete this.els.root.dataset.idle;
        delete this.els.root.dataset.navigating;
        delete this.els.root.dataset.colorBrightness;
        delete this.els.root.dataset.zoomedIn;
    ...
    makeOptions(options, defaults) {
        ...
        for (let key in options) opts[key] = options[key] ?? defaults[key];
    ...
    setColor(color) {
        this.els.root.dataset.colorBrightness = getColorBrightness(color);
    ...
    getContentRect(pageSpread) {
        ...
        const imageRatio =
            +pageEl.dataset.height / (+pageEl.dataset.width * pageCount);
    ...
    beforeNavigation(e) {
        ...
        this.els.root.dataset.navigating = true;
    ...
    afterNavigation(e) {
        ...
        this.els.root.dataset.navigating = false;
    ...
    clicked(e) {
        if (e.isInsideContent) {
            const pageId = e.pageEl.dataset.id;
    ...
    getPageMode() {
        return (
            ...
            (this.els.root.offsetHeight / this.els.root.offsetWidth < 0.8
                ? 'double'
                : 'single')
        );
    }
    ...
    startIdleTimer() {
        this.idleTimeout = setTimeout(() => {
            this.els.root.dataset.idle = true;
