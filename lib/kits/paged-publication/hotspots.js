...
class PagedPublicationHotspots extends MicroEvent {
    ...
    renderHotspots({versoPageSpread, pageSpread, hotspots, pages, ratio}) {
        ...
        for (let id in hotspots) {
            ...
            const position = this.getPosition(pages, ratio, hotspot);
    ...
    renderHotspot(hotspot, position, contentRect, boundingRect) {
        ...
        if (hotspot.id) el.setAttribute('data-id', hotspot.id);

        if (hotspot.type) el.setAttribute('data-type', hotspot.type);
    ...
    getPosition(pages, ratio, hotspot) {
        ...
        for (var pageNumber in hotspot.locations) {
            ...
            hotspot.locations[pageNumber].forEach((coords) => {
                ...
                if (pages[1] && pageNumbers[1] === +pageNumber) x += 1;
    ...
    requestHotspots(pageSpreadId, pages) {
        this.trigger('hotspotsRequested', {id: pageSpreadId, pages});
    }
