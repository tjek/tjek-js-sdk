...
export default class Bootstrapper {
    constructor(options = {}) {
        ...
        this.pointer = matchMedia('(pointer:coarse)').matches
            ? 'coarse'
            : 'fine';
        this.orientation =
            screen.width >= screen.height ? 'horizontal' : 'vertical';
    ...
    async fetch(callback) {
        try {
            const {0: details, 1: incito} = await Promise.all([
                ...
                this.fetchIncito(details.incito_publication_id)
            ]);
