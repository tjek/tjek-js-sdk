...
export default class Bootstrapper {
    ...
    async fetch(callback) {
        try {
            const details = await this.fetchDetails(this.options.id);

            const incito = await this.fetchIncito(
                details.incito_publication_id
            );

            if (typeof callback === 'function') {
                callback(null, {details, incito});
            }

            return {details, incito};
        } catch (err) {
            if (typeof callback === 'function') {
                callback(err);
            } else {
    ...
    fetchDetails = (id, callback) =>
        request({url: `/v2/catalogs/${id}`}, callback);
