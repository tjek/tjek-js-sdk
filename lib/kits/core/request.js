...
import Tjek from '../../tjek-sdk';
import {error, promiseCallbackInterop} from '../../util';

function request(options = {}, callback) {
    let url = Tjek.config.get('coreUrl') + (options.url ?? '');
    const method = options.method || 'get';
    const headers = options.headers ?? {};
    const apiKey = Tjek.config.get('apiKey');
    const clientVersion = Tjek.config.get('clientVersion');
