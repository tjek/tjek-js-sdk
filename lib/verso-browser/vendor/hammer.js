...
function boolOrFn(val, args) {
    if (typeof val == 'function') return val.apply(args?.[0], args);
 ...
 * @returns {*}
 */
const ifUndefined = (val1, val2) => (val1 === undefined ? val2 : val1);
 ...
 */
const inStr = (str, find) => str.indexOf(find) > -1;
 ...
 * @returns {Array} words
 */
const splitStr = (str) => str.trim().split(/\s+/g);
 ...
 * @constructor
 */
export function Input(manager, callback) {
    ...
    this.domHandler = (ev) => {
        if (boolOrFn(manager.options.enable, [manager])) this.handler(ev);
    };
 ...
 */
function simpleCloneInputData({deltaX, deltaY, pointers}) {
 ...
 */
const getDistance = (p1, p2, {0: xKey, 1: yKey} = PROPS_XY) =>
    Math.sqrt(
...
inherit(MouseInput, Input, {
     ...
     */
    handler(srcEvent) {
        let eventType = MOUSE_INPUT_MAP[srcEvent.type];

        // on start we want to have the left mouse button down
        if (eventType & INPUT_START && srcEvent.button === 0) {
            this.pressed = true;
        }

        if (eventType & INPUT_MOVE && srcEvent.which !== 1) {
            eventType = INPUT_END;
        }
...
inherit(PointerEventInput, Input, {
    ...
    handler(srcEvent) {
        ...
        const eventTypeNormalized = srcEvent.type
            .toLowerCase()
            .replace('ms', '');
        const eventType = POINTER_INPUT_MAP[eventTypeNormalized];
        const pointerType =
            IE10_POINTER_TYPE_ENUM[srcEvent.pointerType] ||
            srcEvent.pointerType;

        const isTouch = pointerType == INPUT_TYPE_TOUCH;

        // get index of the event in the store
        let storeIndex = store.findIndex(
            (item) => item.pointerId == srcEvent.pointerId
        );

        let removePointer = false;
        // start and mouse must be down
        if (eventType & INPUT_START && (srcEvent.button === 0 || isTouch)) {
            if (storeIndex < 0) {
                store.push(srcEvent);
...
inherit(SingleTouchInput, Input, {
    handler(srcEvent) {
        const type = SINGLE_TOUCH_INPUT_MAP[srcEvent.type];
        ...
        const [pointers, changedPointers] = normalizeSingleTouches(
            srcEvent,
            type
        );
...
const PREFIXED_TOUCH_ACTION = () => {
    const te = typeof document != 'undefined' && document.createElement('div');
...
function cleanTouchActions(actions) {
    ...
    return inStr(actions, TOUCH_ACTION_MANIPULATION)
...
Recognizer.prototype = {
    ...
    emit(input) {
        ...
        const emit = (event) => this.manager.emit(event, input);

        // 'panstart' and 'panmove'
        if (state < STATE_ENDED) {
            emit(this.options.event + stateStr(state));
        }
 ...
 * @constructor
 */
export function Manager(element, options) {
...
function toggleCssProps(manager, add) {
    ...
    Object.entries(manager.options.cssProps).forEach(({0: value, 1: name}) => {
...
export const Tap = TapRecognizer;
export const Pan = PanRecognizer;
export const Pinch = PinchRecognizer;
export const Press = PressRecognizer;
