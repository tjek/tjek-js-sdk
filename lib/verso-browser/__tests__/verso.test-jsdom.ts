...
test('Page spreads getting their active state set', () => {
    expect(
        document.querySelector<HTMLElement>('[data-id=page1]')!.dataset.active
    ).toBe('true');
});

test('Page spreads getting their left value set relative to each other', () => {
    expect(
        document.querySelector<HTMLElement>('[data-id=page1]')!.style.left
    ).toBe('0%');
    expect(
        document.querySelector<HTMLElement>('[data-id=page2]')!.style.left
    ).toBe('80%');
});
...
test('Navigation to next page', () => {
    ...
    expect(
        document.querySelector<HTMLElement>('[data-id=page2]')!.dataset.active
    ).toBe('true');
    expect(
        document.querySelector<HTMLElement>('[data-id=page1]')!.style.left
    ).toBe('0%');
    ...
    expect(
        document.querySelector<HTMLElement>('[data-id=page3]')!.style.left
    ).toBe('180%');
});
