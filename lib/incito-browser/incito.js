...
export default class Incito extends MicroEvent {
    constructor(containerEl, {incito = {}}) {
        ...
        this.ids = {};
        this.sections = [];
        this.isLazyloadingOn = 'IntersectionObserver' in window;
        this.render();
    }
    ...
    start() {
        ...
        if (this.isLazyloadingOn) {
            this.el.querySelectorAll('.incito--lazy').forEach((el) => {
                this.lazyObserver.observe(el);
            });
            this.el.querySelectorAll('.incito__video-view[data-autoplay=true]').forEach((el) => {
