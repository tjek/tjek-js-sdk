...
const isDefinedStr = (value) => typeof value === 'string' && value.length > 0;
...
const formatSpans = (text, spans) => {
    ...
    spans.forEach((span, i) => {
        ...
        result.push({text: text.slice(startIndex, endIndex), span});
    ...
    return result
        .map((item) => {
            ...
            if (item.span?.name) {
                ...
                return `<span data-name="${item.span.name}">${escapedText}</span>`;
            }
...
function renderView(view, canLazyload) {
    ...
    if (view.view_name === 'TextView') {
        ...
        if (isDefinedStr(view.text_shadow)) {
            ...
            styles['text-shadow'] = `${dx}px ${dy}px ${radius}px ${color}`;
        }
    ...
    if (isDefinedStr(view.shadow_color)) {
        ...
        styles['box-shadow'] = `${dx}px ${dy}px ${radius}px ${color}`;
    }
    ...
    if (
        ...
        styles['transform-origin'] = view.transform_origin
            .map(formatUnit)
            .join(' ');
    }

    return {tagName, contents, classNames, styles, attrs};
}
...
export default class Incito extends MicroEvent {
    ...
    render() {
        ...
        if (this.incito.font_assets) {
            ...
            for (const key in this.incito.font_assets) {
                const value = this.incito.font_assets[key];
    ...
    renderHtml(view) {
        ...
        try {
            const {tagName, contents, classNames, styles, attrs} = renderView(
                view,
                this.canLazyload
            );
            ...
            if (view.role === 'section') {
                this.sections.push({id: view.id, meta: view.meta});
            }

            html += `<${tagName}`;
            html += ` class="${classNames.join(' ')}"`;

            for (const key in attrs) html += ` ${key}="${attrs[key]}"`;

            html += ' style="';

            for (const key in styles) html += `${key}:${styles[key]}; `;
            ...
            if (Array.isArray(view.child_views)) {
                html += view.child_views.reduce(
                    (memo, childView) => memo + this.renderHtml(childView),
                    ''
                );
            }
